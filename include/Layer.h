...
public:
  ...
  double maxZwithHybrids() const { double max = 0; for (const auto& r : rods_) { max = MAX(max, r.maxZwithHybrids()); } return max; }
  double minZwithHybrids() const { double min = std::numeric_limits<double>::max(); for (const auto& r : rods_) { min = MIN(min, r.minZwithHybrids()); } return min; }
  double maxRwithHybrids() const { double max = 0; for (const auto& r : rods_) { max = MAX(max, r.maxRwithHybrids()); } return max; }
  double minRwithHybrids() const { double min = std::numeric_limits<double>::max(); for (const auto& r : rods_) { min = MIN(min, r.minRwithHybrids()); } return min; }
