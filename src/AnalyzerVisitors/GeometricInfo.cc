...
void TrackerVisitor::preVisit() {
  ...
  output_ << "DetId/U, BinaryDetId/B, Section/C, Layer/I, Ring/I, SensorCenter rho(mm), SensorCenter z(mm), tiltAngle_deg/D, skewAngle_deg/D, yawAngle_deg/D, phi_deg/D, meanWidth_mm/D, length_mm/D, sensorSpacing_mm/D, sensorThickness_mm/D" << std::endl;
}
...
void EndcapVisitor::preVisit() {
  output_ << "DetId, BinaryDetId, Endcap-Disc name, Ring, SensorCenter rho(mm), SensorCenter z(mm), tiltAngle(deg), yawAngle(deg), phi(deg), meanWidth(mm) (orthoradial), length(mm) (radial), sensorSpacing(mm), sensorThickness(mm)" << std::endl;
}
